...
class biclustering
{
	public:
		...
		void scoring();
		boost::python::tuple getbicluster();
	...
	for(int i=0; i<numberOfRows; i++)
	{
		for(int j=0; j<numberOfColumns; j++)
		{
			...
			if (value_string=="NA")
				...
				value = int(atof(value_string.c_str())*100);
...
boost::python::tuple biclustering::getbicluster()
{
	for (int i = 0; i < numberOfRows; i++)
		remainingR[i] = true;
	for (int j = 0; j < numberOfColumns; j++)
	...
	return make_tuple(row_index_list, column_index_list);
}
